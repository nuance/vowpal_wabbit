...
void accumulate(vw& all, regressor& reg, size_t o) {
  ...
  all_reduce(local_grad, length, all.span_server, all.listen_address, all.listen_port, all.unique_id, all.total, all.node, all.socks);
  for(uint32_t i = 0;i < length;i++) 
    {
...
float accumulate_scalar(vw& all, float local_sum) {
  ...
  all_reduce(&temp, 1, all.span_server, all.listen_address, all.listen_port, all.unique_id, all.total, all.node, all.socks);
  return temp;
}
...
void accumulate_avg(vw& all, regressor& reg, size_t o) {
  ...
  weight* weights = reg.weight_vector;
  float numnodes = 1.;
  all_reduce(&numnodes, 1, all.span_server, all.listen_address, all.listen_port, all.unique_id, all.total, all.node, all.socks);
  for(uint32_t i = 0;i < length;i++) 
    {
      local_grad[i] = weights[stride*i+o];
    }
...
void accumulate_weighted_avg(vw& all, regressor& reg) {
  ...
  all_reduce(local_weights, length, all.span_server, all.listen_address, all.listen_port, all.unique_id, all.total, all.node, all.socks);
  ...
  all_reduce(weights, 2*length, all.span_server, all.listen_address, all.listen_port, all.unique_id, all.total, all.node, all.socks);
